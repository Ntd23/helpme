  ...
  return inkWellWidget(
    onTap: () async {
      ...
      if(availableMaps.length>1){
        return showDialog(context: getContext,builder:(context) {
          return AlertDialog(
...
Future<void> exportedLogTest({required String logMessage, required String file_name}) async {
  ...
  final filePath = '${downloadsDirectory.path}/${file_name+"${DateTime.now().hour}_${DateTime.now().minute}_${DateTime.now().second}"}.txt';
  final file = File(filePath);
  try {
    await file.writeAsString(logMessage, mode: FileMode.append);
  } catch (e) {
  ...
  OneSignal.Notifications.addClickListener((notification) async {
    ...
    if (notId != null) {
      if (notId.toString().contains('CHAT')) {
        ...
        launchScreen(getContext, ChatScreen(userData: user.data,),isNewTask: true);
      }
    }
  });
}
