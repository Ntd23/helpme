...
Future<Response> buildHttpResponse(String endPoint, {HttpMethod method = HttpMethod.GET, Map? request}) async {
  print("API_CALL_DETECTED :::${endPoint}");
  if (await isNetworkAvailable()) {
    var headers = buildHeaderTokens();
    Uri url = buildBaseUrl(endPoint);
    await exportedLog(file_name: "R_request_${method.name}",logMessage: "$endPoint ${request}");
    print('NETWORK_LOG-Header:${headers.toString()}');

    try {
      Response response;
      print('NETWORK_LOG-Request: $request URL_IS:$url REQ_TYPE:$method');
      ...
      print('NETWORK_LOG-Response ($method): ${url.toString()} ${response.statusCode} ${response.body}');
      await exportedLog(file_name: "R_response_${method.name}",logMessage: "$endPoint ${response.statusCode}\n${response.body}");
      return response;
    } catch (e) {
      await exportedLog(file_name: "R_catch_${method.name}",logMessage: "$endPoint ${request}\n Error:$e");
      // throw e;
